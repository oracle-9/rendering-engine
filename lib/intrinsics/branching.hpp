...
template <std::convertible_to<long> Expr>
auto constexpr likely(Expr&& expr)
    noexcept(std::is_nothrow_convertible_v<Expr, long>)
-> bool
{
    return __builtin_expect(static_cast<long>(std::forward<Expr>(expr)), 1l);
}
...
-> bool
{
    return __builtin_expect(static_cast<long>(std::forward<Expr>(expr)), 0l);
}
